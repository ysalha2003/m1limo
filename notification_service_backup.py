...
class NotificationService:
    ...
    def send_unified_booking_notification(
        ...
        """
        logger.info(f"[UNIFIED NOTIFICATION] Booking: {booking.id}, Event: {event}")
        ...
        # Send to customers (User + Passenger)
        customer_recipients = cls._get_customer_recipients(booking)
        ...
        # Send to admins (based on preferences)
        ...
        total_recipients = len(customer_recipients) + len(admin_recipients)
        notification_sent = len(successful_recipients) > 0
        
        logger.info(f"[UNIFIED NOTIFICATION END] Booking: {booking.id}, Sent: {notification_sent}, Success: {len(successful_recipients)}/{total_recipients}")
    ...
    @classmethod
    def _get_customer_recipients(cls, booking: Booking) -> list:
    ...
    @classmethod
    def _get_admin_recipients(cls, booking: Booking, event: str) -> list:
        """Get admin recipients based on event type and preferences."""
        recipients = set()
        
        try:
            # Global admin recipients
            global_recipients = NotificationRecipient.objects.filter(is_active=True)
            
            for recipient in global_recipients:
                if event == 'new' and recipient.notify_new:
                    recipients.add(recipient.email)
                elif event == 'confirmed' and recipient.notify_confirmed:
                    recipients.add(recipient.email)
                elif event == 'cancelled' and recipient.notify_cancelled:
                    recipients.add(recipient.email)
                elif event == 'status_change' and getattr(recipient, 'notify_status_changes', True):
                    recipients.add(recipient.email)
            
            # Booking-specific recipients
            booking_notifications = BookingNotification.objects.filter(
                booking=booking,
                recipient__is_active=True
            ).select_related('recipient')
    ...
    @classmethod
    def _get_all_admin_recipients(cls) -> list:
        """Get all active admin recipients (for driver alerts)."""
        recipients = []
        
        try:
            global_recipients = NotificationRecipient.objects.filter(is_active=True)
            recipients = [r.email for r in global_recipients]
        except Exception as e:
